...
export function getRequestFingerprint(request: NextRequest, sessionId = "chat") {
  ...
  const secret = process.env.CHATBOT_FINGERPRINT_SECRET || `${chatbotConfig.brandName.toLowerCase()}-local-development`;
  return createHash("sha256").update(`${ip}:${sessionId}:${secret}`).digest("hex");
}

type RateEntry = { count: number; resetsAt: number };
const globalRateStore = globalThis as typeof globalThis & { __nexoraChatRates?: Map<string, RateEntry> };

export function checkMemoryRateLimit(fingerprint: string, limit = 15, windowMs = 60_000) {
  const store = globalRateStore.__nexoraChatRates ??= new Map<string, RateEntry>();
  const now = Date.now();
