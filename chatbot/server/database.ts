...
export async function createLeadOnce({ id, answers, summary, source }: { id: string; answers: LeadAnswers; summary: string; source: Record<string, string> }) {
  ...
  const { name: nameId, email: emailId, phone: phoneId, service: serviceId } = chatbotConfig.fieldIds;
  const name = answers[nameId];
  const email = answers[emailId];
  const phone = answers[phoneId];
  const service = answers[serviceId];
  const fingerprint = contactFingerprint(email, phone);
  const qualification = chatbotConfig.integrations.unqualifiedServiceValues.some((value) => value === service) ? "needs_review" : "qualified";
  ...
  const prior = (await sql.query(
    `SELECT * FROM chatbot_leads
     ...
     LIMIT 1`,
    [id, fingerprint, email, phone.replace(/\D/g, "")],
  )) as DatabaseRow[];
  if (prior[0]) {
  ...
  const inserted = (await sql.query(
    `INSERT INTO chatbot_leads (id, session_id, contact_fingerprint, name, email, phone, service, answers, summary, qualification, source)
     ...
     RETURNING *`,
    [id, fingerprint, name, email.toLowerCase(), phone, service, JSON.stringify(answers), summary, qualification, JSON.stringify(source)],
  )) as DatabaseRow[];
