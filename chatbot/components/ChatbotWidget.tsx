...
function isNewInquiryAttempt(content: string) {
  return [
    /\b(?:new|another|second|different)\s+(?:inquiry|request|entry|project|website|service|campaign|automation|app)\b/i,
    /\b(?:start|submit|send)\s+(?:over|again|another)\b/i,
    /\b(?:change|edit|update)\s+(?:my\s+)?(?:inquiry|request|details)\b/i,
    /\b(?:i|we)\s+(?:need|want|would like|are looking for)\b.{0,60}\b(?:website|software|service|system|project|seo|marketing|ads|automation|branding|ecommerce|app)\b/i,
...
export default function ChatbotWidget() {
  ...
  return (
    <div className="fixed bottom-0 right-0 z-[2147483647] isolate">
      {open && (
        <section className="fixed bottom-[78px] right-3 flex h-[min(620px,calc(100dvh-104px))] w-[calc(100vw-24px)] max-w-[390px] flex-col overflow-hidden rounded-[20px] border border-secondary/35 bg-[#080808] shadow-[0_28px_90px_rgba(0,0,0,0.72),0_0_42px_rgba(255,75,0,0.12)] sm:right-[18px]" role="dialog" aria-labelledby="nexora-chat-title">
          ...
          {quickActions.length > 0 && <div className="flex gap-1.5 overflow-x-auto border-t border-secondary/20 bg-[#0b0b0b] px-3.5 py-2.5">{quickActions.map((action) => <button key={action} type="button" disabled={busy} onClick={() => sendMessage(action)} className="shrink-0 rounded-full border border-secondary/25 bg-white/[0.04] px-3 py-1.5 text-[10px] font-bold text-white/70 transition-colors hover:border-secondary hover:bg-secondary hover:text-black disabled:opacity-50">{action}</button>)}</div>}
          {error && <div className="border-t border-amber-500/25 bg-amber-500/10 px-4 py-2 text-center text-[10px] font-medium text-amber-200" role="alert">{error}</div>}
          <ChatComposer value={input} onChange={setInput} onSubmit={() => sendMessage()} busy={busy} />
        </section>
      )}
      {!open && <div className="fixed bottom-[36px] right-3 flex items-center gap-2 sm:bottom-[32px] sm:right-[18px]"><span className="hidden rounded-full border border-secondary/25 bg-black/90 px-3 py-2 text-[10px] font-bold text-white/70 shadow-[0_10px_28px_rgba(0,0,0,0.45)] backdrop-blur-md sm:flex sm:items-center sm:gap-1.5"><Sparkles className="h-3 w-3 text-secondary" />Ask {chatbotConfig.assistantName}</span><button type="button" onClick={() => setOpen(true)} className="group relative flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-primary-1 via-primary-2 to-secondary text-black shadow-[0_16px_38px_rgba(255,75,0,0.34)] transition-transform hover:-translate-y-1 sm:h-[52px] sm:w-[52px]" aria-label={`Open ${chatbotConfig.brandName} chat`} aria-expanded={open}><span className="absolute inset-0 animate-ping rounded-full bg-primary-2 opacity-15" /><MessageCircle className="relative h-5 w-5 fill-black/10" /></button></div>}
      {open && <button type="button" onClick={() => setOpen(false)} className="fixed bottom-[36px] right-3 flex h-12 w-12 items-center justify-center rounded-full border border-secondary/25 bg-[#111] text-white shadow-[0_16px_36px_rgba(0,0,0,0.45)] transition-transform hover:-translate-y-1 sm:bottom-[32px] sm:right-[18px] sm:h-[52px] sm:w-[52px]" aria-label={`Close ${chatbotConfig.brandName} chat`}><X className="h-5 w-5" /></button>}
    </div>
  );
}
