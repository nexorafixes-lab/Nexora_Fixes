...
export function InteractiveGlobe({
  ...
  const draw = useCallback(() => {
    ...
    glowGrad.addColorStop(0, "rgba(255, 122, 0, 0.06)");
    ...
    for (const dot of dotsRef.current) {
      ...
      const dotSize = 0.9 + depthAlpha * 0.75;
    ...
    for (const marker of markers) {
      ...
      ctx.strokeStyle = markerColor.replace("1)", `${0.18 + pulse * 0.16})`);
      ctx.lineWidth = 1;
      ctx.stroke();
